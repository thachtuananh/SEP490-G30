...
import { fetchCustomerAddresses, setDefaultAddress } from "../../services/owner/OwnerAddressAPI";
...
const ServiceDetailsCombo = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const detailsPromises = selectedServices.map(serviceId => fetchServiceDetails(serviceId));
                ...
                detailsResults.forEach((serviceData, index) => {
                    ...
                    if (serviceData && serviceData.serviceDetails && serviceData.serviceDetails.length > 0) {
                        ...
                        form.setFieldsValue({ [`area_${serviceId}`]: defaultDetail.serviceDetailId });
                    } else {
            ...
            navigate("/createjobtocleaner", {
                state: {
                    ...
                    serviceDetails: selectedServices.map(serviceId => {
                        const serviceData = servicesDetails.find(s => s.serviceId === serviceId);
                        let selectedDetail = serviceData?.serviceDetails?.find(detail => detail.serviceDetailId === serviceSizes[serviceId]);
                        ...
                        return {
                            ...
                            selectedSize: selectedDetail ? `${selectedDetail.minRoomSize}` : "0",
    ...
    const handleSetDefaultAddress = async (addressId) => {
        try {
            ...
            const updatedDefaultAddress = customerAddresses.find(addr => addr.addressId === addressId);
    ...
    const calculateTotalPrice = () => {
        return Object.values(servicePrices).reduce((total, price) => total + price, 0);
    };
                            ...
                            return (
                                <div key={serviceId} className={styles.selectedServiceItem}>
                                    <div className={styles.selectedServiceHeader}>
                                        <span>{service?.title || serviceData?.serviceName || `Dịch vụ ${serviceId}`}</span>
                                        <Button
                                            type="text"
                                            danger
                                            icon={<span>🗑️</span>}
                                    ...
                                    <div className={styles.serviceAreaSelection}>
                                        ...
                                        <Form.Item name={`area_${serviceId}`} noStyle>
                                            {serviceData && serviceData.serviceDetails && serviceData.serviceDetails.length > 0 ? (
                                                <Radio.Group
                                                    onChange={(e) => handleSizeChange(serviceId, e.target.value)}
                                                    defaultValue={serviceData.serviceDetails[0].serviceDetailId}
                                                >
                                                    {serviceData.serviceDetails.map(detail => (
                                                        <Radio.Button key={detail.serviceDetailId} value={detail.serviceDetailId}>
                                                ...
                                                <Radio.Group defaultValue="default">
                                                    <Radio.Button value="default">{"< 20m²"}</Radio.Button>
                                                </Radio.Group>
                                            )}
                                        </Form.Item>
